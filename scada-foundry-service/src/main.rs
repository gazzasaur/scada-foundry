...
use anyhow::Error;
use axum::{
    Json, Router, extract::{ConnectInfo, ws::{Message, Utf8Bytes, WebSocket, WebSocketUpgrade}}, http::StatusCode, response::IntoResponse, routing::{any, get, post},
};
use clap::Parser;
...
async fn main() -> Result<(), Error> {
    ...
    let app = Router::new().route("/", get(root)).route("/users", post(create_user)).route("/app/ws", any(ws_handler));
